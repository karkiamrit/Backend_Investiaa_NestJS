...
export class TokenRepository {
   ...
   */
    async createToken(user: User, ttl: number): Promise<Token> {
        ...
        const token: Token = await Token.create({ user, expires_in: expiresIn }).save()
        return token
    }
    ...
    async deleteToken(user: User, tokenId: number): Promise<boolean> {
        return await Token.update({ user: { id: user.id }, id: tokenId }, { is_revoked: true }) !== null
    }
    ...
    async deleteTokensForUser(user: User): Promise<boolean> {
        return await Token.update({ user: { id: user.id } }, { is_revoked: true }) !== null
    }
}
